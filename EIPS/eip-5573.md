...
- `att`: (CONDITIONAL) If present, `att` MUST be a JSON object where each key is a URI and each value is an object containing namespaced-actions as keys and a corrosponding value which is an array of qualifications to the action (i.e. a restriction or requirement). The keys of the object MUST be ordered lexicographically.
- `prf`: (CONDITIONAL) If present, `prf` MUST be a JSON array of string values with at least one entry where each value is a valid Base58-encoded CID which identifies a parent capability, authorising one or more of the entries in `att` if the SIWE `address` does not identify the controller of the `att` entries.

Objects in the `att` field (including nested objects) MUST have their keys ordered lexicographically with two steps:
...
This is the same as the `Array.sort()` method in Javascript. In the example below, `crud/delete` must appear before `crud/update` and `other/action`, similarly `msg/recieve` must appear before `msg/send`.
