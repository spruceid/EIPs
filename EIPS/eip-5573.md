...
Objects in the `att` field (including nested objects) MUST NOT contain duplicate keys and MUST have their keys ordered lexicographically according to RFC8785 JSON Canonicalization Scheme. This entails two steps:
...
This is the same as the default `Array.sort()` method behaviour in Javascript. In the example below, `crud/delete` must appear before `crud/update` and `other/action`, similarly `msg/receive` must appear before `msg/send`.
