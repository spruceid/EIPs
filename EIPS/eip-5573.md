...
The objects which appear as values in the `att` object MUST have their fields ordered lexicographically. In the example below, `crud/delete` must appear before `crud/update` and `other/action`, similarly `msg/recieve` must appear before `msg/send`.
