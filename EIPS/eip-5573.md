...
- ReCap Details Object - A [DAG-CBOR](https://github.com/ipld/ipld/blob/master/specs/codecs/dag-cbor/spec.md) object describing the actions and optionally the resources associated with a ReCap Capability under a certain namespace.
...
A ReCap URI starts with `urn:recap:` followed by `:` and the base64url-encoded payload of the ReCap Details Object. Note, the term base64url is defined in RFC4648 - Base 64 Encoding with URL and Filename Safe Alphabet.
...
cid = #6.42(bstr)
uri = tstr .regex ^.+:.*$
ability-string = tstr .regex ^[a-zA-Z0-9.*_+-]+\/[a-zA-z0-9.*_+-]+$
...
- `att`: (CONDITIONAL) If present, `att` MUST be a DAG-CBOR object where each key is a URI and each value is an object containing namespaced-actions as keys and a corrosponding value which is an array of qualifications to the action (i.e. a restriction or requirement).
- `prf`: (CONDITIONAL) If present, `prf` MUST be a DAG-CBOR array of string values with at least one entry where each value is a valid Base58-encoded CID which identifies a parent capability, authorising one or more of the entries in `att` if the SIWE `address` does not identify the controller of the `att` entries.

The following is a non-normative example of a ReCap Capability Object with `att` and `prf`, represented as pseudo-JSON:
```jsonc
{
   att: {
...
```text
I further authorize did:key:example to perform the following actions on my behalf: (1) "crud": "delete", "update" for "https://example.com/pictures". (2) "other": "action" for "https://example.com/pictures". (3) "msg": "recieve", "send" for "mailto:username@example.com".
```
...
- For each key and value pair in `attenuations` (starting with the first entry), perform the following:
  ...
  - Group the keys of the `ability` object by their `ability-namespace`
